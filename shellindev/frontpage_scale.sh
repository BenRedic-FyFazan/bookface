...
function scale {
	# This script is assumed to run on the manager and SSH to a server in order to run the docker service upgrade command. 
	# We also assume that we need to use sudo.
	COMMAND="sudo docker service update --env-add BF_FRONTPAGE_LIMIT=$1 bf_web"
	# This could be improved, because what happens when server 1 is unavailable? TODO: Add check for status of $1
